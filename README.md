...
git merge --no-ff //合并时禁用fast forward模式 fast forward模式模式合并时不保留原分支信息 直接并入merge后分支
git reset <分支名>
...
git cherry-pick <HashA> <HashB> //支持一次复制多个提交
git cherry-pick A..B //A早于B才会成功 复制A到B 不包含A的所有提交
git cherry-pick A^..B //包含A的 复制A到B的所有提交
...
git remote -v //查看详细
git remote //查看不详细
git remote rm [简称:origin]  解除远程分支与本地分支的绑定关系
...
**Git的标签虽然是版本库的快照，但其实它就是指向某个commit的指针（跟分支很像对不对？但是分支可以移动，标签不能移动），所以，创建和删除标签都是瞬间完成的。<br>
标签就是跟某个commit关联起来，便于发布和查找<br>
...
ssh -T git@github.com //在本地终端执行以下命令，测试 SSH 连接是否成功
